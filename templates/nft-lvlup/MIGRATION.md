# nft-lvlup — migration plan

Group C: port from `@paima/*` (paima-engine-v1) at
`/home/eddie/paima-game-templates/nft-lvlup` to `@effectstream/*` (Bun 0.100.18).

## Source
- Repo: `/home/eddie/paima-game-templates/nft-lvlup`
- Top-level dirs: `api/`, `contracts/`, `db/`, `frontend/`, `frontend-nft-sale/`
- Dep scope: @paima/{sdk,node-sdk,build-utils}@2.2.0, @paima/evm-contracts@1.1.0
- Build: `npx tsc --build`, npm workspaces (not bun)
- Has an extra `frontend-nft-sale/` package — consider merging into `packages/frontend/` or shipping as `packages/frontend-nft-sale/`.
...
5. `LINK_LOCAL=1 bun run templates/run-template-tests.ts nft-lvlup` exit 0
