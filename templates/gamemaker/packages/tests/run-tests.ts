...
async function test() {
  ...
  try {
    ...
    const { gainedExperienceTest } = await import("./stm/actions.test.ts");
    await gainedExperienceTest(db);
    ...
    // Real end-to-end: drives the local-JS wallet (EvmViem) through the full
    // user flow in headless Chromium — connect → submit gainedExperience →
    // see XP reflected by the API. Only possible because EvmViem needs no
    // browser extension; same shape applies to CardanoLocal + MidnightLocal.
