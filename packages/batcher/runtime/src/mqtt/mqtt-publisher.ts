...
export class MQTTPublisher {
  ...
  private static getClient(): mqtt.MqttClient {
    ...
    // Protocol MQTT
    MQTTPublisher.client_ = mqtt.connect('mqtt://127.0.0.1:1884'); // ENV.MQTT_BROKER_URL);
  ...
  public static async startListener(topic: MQTTSystemEvents): Promise<void> {
    ...
    while (!ready) {
      console.log('Waiting for MQTT client at ', 'mqtt://127.0.0.1:1884'); // ENV.MQTT_BROKER_URL);
      await wait(100);
    }
