...
import { retrieveFee, getPaimaL2ContractOwner, initWeb3 } from 'paima-utils';
...
function App() {
  ...
  const updateValues = async (web3: Web3) => {
    const [fee, owner, balance] = await Promise.all([
      ...
      getPaimaL2ContractOwner(CONTRACT_ADDRESS, web3),
  ...
  const autoUpdateValues = async () => (web3Uninitialized ? null : updateValues(web3));
  ...
  const connectWalletFxn = () => {
    userWalletLogin().then(userWallet => {
      verifyWalletChain().then(chainValidity => setCorrectChain(chainValidity));
  ...
  const withdrawFundsFxn = () => {
    ...
    promiseWithdrawFunds(web3, wallet).then(console.log).catch(console.error);
  };
  ...
  const setFeeFxn = () => {
    ...
    if (/^\d+$/.test(inputString)) {
      promiseSetFee(web3, wallet, inputString).then(console.log).catch(console.error);
    } else {
  ...
  const setOwnerFxn = () => {
    ...
    if (/^0x[0-9a-fA-F]+$/.test(inputString)) {
      promiseSetOwner(web3, wallet, inputString).then(console.log).catch(console.error);
    } else {
