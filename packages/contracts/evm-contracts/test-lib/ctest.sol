...
contract CTest {
  ...
  function assertTrue(bool condition, string memory err) internal returns (bool) {
  ...
  function assertEq(string memory a, string memory b, string memory err) internal {
    if (!assertTrue(keccak256(abi.encodePacked(a)) == keccak256(abi.encodePacked(b)), err)) {
  ...
  function checkEq0(bytes memory a, bytes memory b) internal pure returns (bool) {
