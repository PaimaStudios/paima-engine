...
async function getEntryFromFile(namespace: string, blockHeight: number): Promise<undefined | string[]> {
...
export async function getReadNamespaces(blockHeight: number): Promise<string[]> {
    ...
    if (namespace != null && namespace !== '') {
        const entry = namespace.endsWith('.yml') || namespace.endsWith('.yaml')
        ...
        const adjusted = entry.map(prefix => prefix === 'CONTRACT_ADDRESS' ? ENV.CONTRACT_ADDRESS : prefix);
        return adjusted;
    } else {
        return [ENV.CONTRACT_ADDRESS];
    }
